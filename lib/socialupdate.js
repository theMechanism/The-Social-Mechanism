...
var twitterConsumerKey = require('/var/www/vhosts/themechanism.com/httpdocs/playground/thesocialmechanism/lib/private.js').twitterConsumerKey;
var twitterConsumerSecret = require('/var/www/vhosts/themechanism.com/httpdocs/playground/thesocialmechanism/lib/private.js').twitterConsumerSecret;
var twitterAccessToken = require('/var/www/vhosts/themechanism.com/httpdocs/playground/thesocialmechanism/lib/private.js').twitterAccessToken;
var twitterAccessSecret = require('/var/www/vhosts/themechanism.com/httpdocs/playground/thesocialmechanism/lib/private.js').twitterAccessSecret;
...
var addLoop = function(r, res, social_collection, callback) {
	...
	social_collection.save(res[r], {safe:true},
		function(error, saved) {
			if(error||!saved) {
				callback("Save error " + error + " : " + saved);
			}
				...
				response.on('end', function (chunk) {
					try {
						...
						for (v in feed.entry) {
							...
							var liked = "0";
							if (video.yt$rating && video.yt$rating.numLikes) {
								liked = video.yt$rating.numLikes;
							}
							
							var linker = "#";
							for (l in video.link) {
								if (video.link[l].rel == "alternate") {
									linker = video.link[l].href;
								}
							}
							
							// Find default and hqdefault thumb images
							var tURL = "";
							var lURL = "";
							for( var prop in video.media$group.media$thumbnail ) {
								if (video.media$group.media$thumbnail[prop].yt$name == "default") {
									tURL = video.media$group.media$thumbnail[prop].url;
								}
								if (video.media$group.media$thumbnail[prop].yt$name == "hqdefault") {
									lURL = video.media$group.media$thumbnail[prop].url;
								}
						    }
						    // If no large image found, set to default
						    lURL = lURL ? lURL : tURL;
						...
						console.log("Error adding videos: " + err.message);
						process.exit(0);
					}
				});
			});
