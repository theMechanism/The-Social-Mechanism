...
module.exports = {
	getTwitter: function(twittername, next) {
		...
		twit.getUserTimeline({
			...
			socialUpdate.saveData(twees, function(data) {
				next(twees);
			});
...
socialUpdate.loggit('Twitter call starting');
socialUpdate.init('social_yotd', twitternames, function(ID, checkIt) {
	module.exports.getTwitter(ID, function() {
		console.log('Twitter data saved for ' + ID);
		var isEnd = checkIt();
		if (isEnd) {
			socialUpdate.loggit('Twitter call finished', function() {
				process.exit(0);
			});
		}
	});
